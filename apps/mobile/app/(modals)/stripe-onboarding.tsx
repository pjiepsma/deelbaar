...
export default function StripeOnboardingModal() {
  ...
  const [WebViewComponent, setWebViewComponent] = useState<ComponentType<
  ...
  useEffect(() => {
    import('react-native-webview')
      .then((m) => setWebViewComponent(() => m.default))
      .catch((err) => {
        console.warn('[StripeOnboarding] react-native-webview failed to load', err);
      });
  }, []);
  ...
  useEffect(() => {
    if (clientSecret && isWebViewLoaded) {
      webViewRef.current?.postMessage(JSON.stringify({ type: 'START_ONBOARDING', clientSecret }));
    }
      ...
      try {
        ...
        switch (data.type) {
          ...
          case 'STEP':
            ...
            break;
          case 'ERROR':
            setWebMessage(data.message || 'Onbekende fout vanuit Stripe onboarding.');
            break;
          default:
            break;
        }
      ...
      <ScrollView
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            Voltooi de onboarding om betalingen te ontvangen en uitbetalingen te laten doen via
            Stripe.
          </Text>
        </View>
        ...
        {!publishableKey ? (
          <View style={styles.alertBox}>
            ...
            <Text style={styles.alertText}>
              Geen Stripe publishable key gevonden. Voeg {`'EXPO_PUBLIC_STRIPE_PUBLISHABLE_KEY'`}{' '}
              toe aan je .env bestand en start de app opnieuw.
            </Text>
          </View>
        ) : null}
          ...
          <View style={styles.webViewContainer}>
            ...
            <View style={styles.webViewWrapper}>
              {WebViewComponent ? (
                ...
                <View style={styles.webViewLoading}>
                  <ActivityIndicator size="large" color={Colors.primary} />
                  <Text style={styles.webViewLoadingText}>WebView laden…</Text>
                </View>
              )}
            </View>
          </View>
