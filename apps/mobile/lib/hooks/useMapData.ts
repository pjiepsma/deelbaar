...
export function useMapData(region: Region) {
  ...
  const currentBounds = useMemo(() => {
    ...
    // Clamp to valid WGS84 ranges - API rejects lat outside [-90,90] or lon outside [-180,180]
    const rawNorthEastLat = region.latitude + latHalf;
    const rawSouthWestLat = region.latitude - latHalf;
    const rawNorthEastLon = region.longitude + lonHalf;
    const rawSouthWestLon = region.longitude - lonHalf;

    return {
      northEast: {
        lat: Math.min(90, rawNorthEastLat),
        lon: Math.min(180, rawNorthEastLon),
      },
      southWest: {
        lat: Math.max(-90, rawSouthWestLat),
        lon: Math.max(-180, rawSouthWestLon),
      },
    };
