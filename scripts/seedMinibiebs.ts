 ...
 * Run with: npx tsx scripts/seedMinibiebs.ts
 */
...
const listings = apeldoornMinibiebs.map((minibieb: any, index: number) => {
  ...
  return {
    ...
    description: minibieb.properties.Bijzonderheid || 'Een minibieb waar je gratis boeken kunt ruilen en delen met anderen in de buurt.',
    location: {
      address: `${minibieb.properties.Adres || ''}, ${minibieb.properties.Plaatsnaam || 'Apeldoorn'}`.trim(),
